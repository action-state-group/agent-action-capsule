...
Defined in the Internet-Draft's Cross-record references section (`-03` and
later revisions). Governs the `citation_purpose` field of a `references[]`
entry — a Capsule's citation of a record outside its own `chain`/`order`
scope (a different producer, stream, or order). Distinct from, and never a
repurposing of, CPB's own `purpose` field on a typed digest reference
