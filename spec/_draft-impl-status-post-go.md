...
Implementation URL:
: https://github.com/agent-action-capsule/agent-action-capsule
...
Level of maturity:
: Implemented and deployed (version 0.0.1).  Implemented against
  draft-mih-scitt-agent-action-capsule-00.  The implementation has been
  validated by two-runtime differential fuzzing against an independently
  written verifier; zero divergences were found across all Class 1 check
  paths.  No known conformance failures.

Coverage:
: Class 1 verification — complete, two-runtime differential-fuzz validated,
  zero known divergences.  Class 2 verification (manifest-dependent material,
  Section 9 ({{class2}})) — not implemented (manifest discovery is out of
