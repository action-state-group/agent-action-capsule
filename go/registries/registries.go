...
// authoritativeRegistry is generated byte-for-byte from spec/REGISTRY.md.
// The drift test requires regeneration whenever the source registry changes.
//
...
// LoadAuthoritative returns AAC's current registry table. Callers that need a
// pinned historical table should use Load with that snapshot's path instead.
func LoadAuthoritative() (map[string]map[string]bool, error) {
	return parse(bytes.NewReader(authoritativeRegistry))
}

// Load parses a pinned REGISTRY.md at path and returns
// {registry_name: set_of_seeded_values} for the profile registries. An empty
// path uses AAC's embedded authoritative table for compatibility with callers
